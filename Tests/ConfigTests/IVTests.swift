...
class IVTests: XCTestCase {
    func testItWritesAPropertyDeclarationCorrectly() throws {
        ...
        expect(iv.propertyDeclaration(for: "", iv: iv, encryptionKey: nil, requiresNonObjCDeclarations: false, isPublic: false, indentWidth: 0)).to(equal("    static let encryptionKeyIV: [UInt8] = \(expectedByteArray)"))
        expect(iv.propertyDeclaration(for: "", iv: iv, encryptionKey: nil, requiresNonObjCDeclarations: false, isPublic: true, indentWidth: 0)).to(equal("    public static let encryptionKeyIV: [UInt8] = \(expectedByteArray)"))
    }
}
